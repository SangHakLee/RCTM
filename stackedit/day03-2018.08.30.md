...
3. copy hosts
	...
	``` 
> ansible 코드 자체의 오류 hostname 문제 
> 원래는 `control.example.com control` 이 문장에서 두번째 축약형 `control`이 제대로 실행되어야 하나 그렇지 못해 발생한 문제
> `$ hostnamectl set-hostname control` 명령어오 제대로 작동하게 만들어준다.
...
```
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTEzNjA1ODU5NTQsMTQzNjExMDI3OCwyMD
MzMzM1NDg4LDE3NDgwNTcyNjEsNTUyNDM1NzM1XX0=
-->